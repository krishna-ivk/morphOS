...
from skyforce.runtime.agents import fix_application_agent
...
def test_fix_application_agent_writes_fix_artifact(repo_root, tmp_path):
    run_dir = tmp_path / "run"
    (run_dir / "artifacts").mkdir(parents=True, exist_ok=True)
    diagnosis = {
        "diagnoses": [
            {
                "test_name": "tests::broken",
                "symptom": "forced failure",
                "fix_strategy": "adjust implementation",
            }
        ]
    }
    (run_dir / "artifacts" / "debug_diagnosis.json").write_text(
        json.dumps(diagnosis, indent=2) + "\n"
    )
    result = fix_application_agent(run_dir)
    assert result["count"] == 1
    applied = json.loads((run_dir / "artifacts" / "applied_fixes.json").read_text())
    assert applied["fixes"][0]["status"] == "applied_placeholder"
